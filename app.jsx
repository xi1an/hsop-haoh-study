...
function App() {
  ...
  useEffect(() => {
    function handleContentUpdate() {
      setApps([...(window.hsopApps || [])]);
      setToast(
        window.HSOP_CONTENT_SOURCE === "network"
          ? "资料已同步到最新版本"
          : "已使用本地缓存资料"
      );
    }

    window.addEventListener("hsop-content-updated", handleContentUpdate);
    return () => window.removeEventListener("hsop-content-updated", handleContentUpdate);
  }, []);
      ...
      <header className="topbar">
        ...
        <div className="topbar-actions">
          <button
            ...
            type="button"
            onClick={() => {
              if (!window.HSOP_REMOTE_CONTENT?.refreshPackage) {
                setToast("当前版本不支持远程刷新");
                return;
              }
              setToast("正在刷新资料...");
              window.HSOP_REMOTE_CONTENT.refreshPackage().catch(() => {
                setToast("远程资料暂时不可用，继续使用本地版本");
              });
            }}
          >
            刷新资料
...
function buildEbankGroups(currentGuide, ebankAction) {
  ...
  return [
    ...groups,
    {
      ...
      title: "网银设备",
      icon: "device",
      items: [{ text: "网银U盾原件", note: "办理网银变更或注销时需携带" }],
    },
  ];
}
...
function buildEbankNotes(currentGuide, ebankAction) {
  ...
  if (ebankAction === "change" || ebankAction === "cancel") {
    return ["办理网银变更或注销时，请携带网银U盾原件。", ...notes];
  }
  return notes;
}
